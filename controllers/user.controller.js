...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ApiError(400, "All filed are compulsory or required");
  }
  ...
  if (
    ...
    coverImageLocalPath = registerUser.files[0].path;
  }
    ...
    return res
      ...
      .json(
        new ApiResponse(
          ...
          "Acces Token is refreshed successfully"
        )
      );
  } catch (error) {
...
const updateUserAvatar = asyncHandler(async (req,res) => {
   ...
   if(!avatar.url){
    ...
    response.status(200).json(
        new ApiResponse(200,
            user,
            "Cover Image updtaed successfully"
        )
    )
})
const updateCoverImage = asyncHandler(async (req,res) => {
   ...
   if(!coverImage.url){
    ...
    const prevUrl = req.user?.coverImage;
    if(prevUrl)
       deleteFromCloudinary(prevUrl);
    const user = await User.findByIdAndUpdate(
        req.user?._id,
...
const getUserChannelProfile = asyncHandler(async (req,res) => {
     ...
     // User.find({username})   or we can do
...
//If you want to update files then create new controllers
